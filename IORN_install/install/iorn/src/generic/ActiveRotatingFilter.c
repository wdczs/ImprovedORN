    ...
    THTensor *output)
{
    ...
    // weight.size() =  nOutputPlane, nInputPlane, nOrientation, kH, kW
    //                       20            10            8        3   3
    // 
    #pragma omp parallel for private(i, j, l, k)
    for (i = 0; i < nOutputPlane; i++) {// 20
        for (j = 0; j < nInputPlane; j++) {// 10
            for (l = 0; l < nEntry; l++) {// 8 x 3 x 3 = 72
                real val = *(weightData++);
                for (k = 0; k < nRotation; k++) {// 8
                    uint8 index = *(indicesData + l * nRotation + k) - 1;// index:0~71
                    real *target = outputData + i * (nRotation * nInputPlane * nEntry)// nEntry = nOrientation * kH * kW
