...
////////////////////////////////////////////////////////////////
template <typename Dtype>
__global__ void AlignFeatureKernel2d(
    const uint32 nthreads, //nBatch * nFeature
    const Dtype* feature_data,
    const uint16 nBatch,
    const uint16 nFeature,
    const uint8 nOrientation,
    const uint16 feature_h,
    const uint16 feature_w,
    uint8* mainDirection_data,
    Dtype* aligned_data) 
{
    CUDA_KERNEL_LOOP(n, nthreads) {
        ...
        Dtype maxVal = -FLT_MAX;
        for (l = 0; l < nOrientation; l++) {
            // base = (feature_data + i * (nFeature * nOrientation * feature_h * feature_w)
...
template <typename Dtype>
__global__ void UnAlignFeatureKernel2d(
    const uint32 nthreads, 
    const Dtype* aligned_data,
    const uint8* mainDirection_data,
    const uint16 nBatch,
    ...
    Dtype* feature_data) 
{
    CUDA_KERNEL_LOOP(n, nthreads) {
        ...
        for (l = 0; l < nOrientation; l++) {
            for (p = 0; p < feature_h * feature_w; p++){
                Dtype src = *(aligned_data + i * (nFeature * nOrientation * feature_h * feature_w)
                                          + j * (nOrientation * feature_h * feature_w)
                                          + l * feature_h * feature_w
                                          + p);
                uint8 alignedIndex = (l + direction) % nOrientation;
                Dtype *target = feature_data + i * (nFeature * nOrientation * feature_h * feature_w)
                                            + j * (nOrientation * feature_h * feature_w)
                                            + alignedIndex * feature_h * feature_w
                                            + p;
                *target = src;                   
...
/////////////////////////////////////////////////////
void kernel_Double_AlignFeature2d(
    cudaStream_t stream,
    const uint32 count,
    const double* feature_data,
    const uint16 nBatch,
    ...
    double* aligned_data)
{
    AlignFeatureKernel2d <<< GET_BLOCKS(count), CUDA_NUM_THREADS, 0, stream >>>
      (count, feature_data, nBatch, nFeature, nOrientation, feature_h, feature_w, mainDirection_data, aligned_data);
}
void kernel_Float_AlignFeature2d(
    cudaStream_t stream,
    const uint32 count,
    const float* feature_data,
    const uint16 nBatch,
...
void kernel_Double_UnAlignFeature2d(
    cudaStream_t stream,
    const uint32 count, 
    const double* aligned_data,
    const uint8* mainDirection_data,
    ...
    double* feature_data)
{
    UnAlignFeatureKernel2d <<< GET_BLOCKS(count), CUDA_NUM_THREADS, 0, stream >>>
      (count, aligned_data, mainDirection_data, nBatch, nFeature, nOrientation, feature_h, feature_w, feature_data);
}
void kernel_Float_UnAlignFeature2d(
    cudaStream_t stream,
    const uint32 count, 
    const float* aligned_data,
    const uint8* mainDirection_data,
